...
void Physics::initMoonVelocity(std::vector<Moon*>& moons)
{
    for(auto moon:moons)
    {
        float u=0;
        CelestialBody* parent = moon->getParent();
        sf::Vector2f r_vector = {moon->x-parent->x,moon->y-parent->y};
        float r = sqrt(pow(r_vector.x,2)+pow(r_vector.y,2));
        if (r<0.00001)
        {
            continue;
        }
        moon->a = r / (1 - moon->eccentricity) ;
        float v_magnitude = sqrt((2*G * parent->mass/r )-(G* parent->mass /moon->a));
        // Relative velocity (moon around parent)
        sf::Vector2f relativeVelocity = {r_vector.y/r * v_magnitude, -1 * r_vector.x/r * v_magnitude};
        
        // Add parent's orbital velocity
        moon->v = {relativeVelocity.x + parent->v.x, relativeVelocity.y + parent->v.y};
